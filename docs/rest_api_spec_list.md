...
| ID | Name | Description | Method | Endpoint | Parameters | Responses |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **PROJ-01** | List Projects | Get all projects visible to the current user. | `GET` | `/projects` | None | **200 OK**: `[ { "id": "uuid", "name": "string", "description": "string", "created_at": "datetime" } ]` |
...
| ID | Name | Description | Method | Endpoint | Parameters | Responses |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **FEAT-01** | List Features | Get all features/requirements for a project with optional pagination. | `GET` | `/projects/{projectId}/features` | **Path**: `projectId`<br>**Query**: `page` (int, default 1), `per_page` (int, default 50), `search` (string, optional) | **200 OK**: `{ "items": [ { "id": "uuid", "name": "string", "description": "string" } ], "total": int, "page": int, "per_page": int }` |
...
| ID | Name | Description | Method | Endpoint | Parameters | Responses |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **COMP-01** | Get Next Pair | Get the next pair of features to compare based on highest information gain (max variance reduction). | `GET` | `/projects/{projectId}/comparisons/next` | **Path**: `projectId`<br>**Query**: `dimension` (enum: complexity, value) | **200 OK**: `{ "comparison_id": "uuid", "feature_a": { "id": "uuid", "name": "string", "description": "string" }, "feature_b": { "id": "uuid", "name": "string", "description": "string" }, "dimension": "string" }`<br>**204 No Content** (if no useful comparisons left) |
| **COMP-02** | Submit Result | Submit the result of a pairwise comparison. | `POST` | `/projects/{projectId}/comparisons` | **Path**: `projectId`<br>**Body**: `{ "comparison_id": "uuid", "choice": "feature_a" \| "feature_b" \| "tie" }` | **200 OK**: `{ "status": "accepted", "updated_variance": float }` |
...
| **COMP-04** | Get Inconsistencies | Get graph cycles representing logical inconsistencies. Returns list of cycles, where each cycle contains the edges (comparisons) involved. | `GET` | `/projects/{projectId}/comparisons/inconsistencies` | **Path**: `projectId`<br>**Query**: `dimension` | **200 OK**: `{ "cycles": [ { "edges": [ { "comparison_id": "uuid", "winner_id": "uuid", "loser_id": "uuid" } ] } ] }` |
...
| **COMP-07** | Get Resolution Pair | Get a specific pair of features to compare to resolve a detected inconsistency. | `GET` | `/projects/{projectId}/comparisons/resolve-inconsistency` | **Path**: `projectId`<br>**Query**: `dimension` | **200 OK**: `{ "comparison_id": "uuid", "feature_a": {...}, "feature_b": {...}, "reason": "cycle_resolution" }`<br>**204 No Content** |
| **COMP-08** | List Comparisons | Get paginated history of past comparisons for audit/review. | `GET` | `/projects/{projectId}/comparisons` | **Path**: `projectId`<br>**Query**: `dimension`, `page` (int), `per_page` (int) | **200 OK**: `{ "items": [ { "id": "uuid", "feature_a": { "id": "uuid", "name": "string" }, "feature_b": { "id": "uuid", "name": "string" }, "choice": "string", "dimension": "string", "created_at": "datetime" } ], "total": int, "page": int }` |
| **COMP-09** | Get Comparison Progress | Get current comparison progress as percentage toward target certainty. | `GET` | `/projects/{projectId}/comparisons/progress` | **Path**: `projectId`<br>**Query**: `dimension`, `target_certainty` (float, default 0.90) | **200 OK**: `{ "dimension": "string", "target_certainty": float, "current_certainty": float, "progress_percent": float, "comparisons_done": int, "comparisons_remaining": int }` |
| **COMP-10** | Undo Last Comparison | Undo the most recent comparison for a dimension. | `POST` | `/projects/{projectId}/comparisons/undo` | **Path**: `projectId`<br>**Body**: `{ "dimension": "string" }` | **200 OK**: `{ "undone_comparison_id": "uuid", "message": "Comparison undone" }`<br>**404 Not Found** (if no comparisons to undo) |
...
| ID | Name | Description | Method | Endpoint | Parameters | Responses |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **RES-01** | Get Ranked Results | Get the final ranked list of features. | `GET` | `/projects/{projectId}/results` | **Path**: `projectId`<br>**Query**: `sort_by` (complexity, value, ratio) | **200 OK**: `[ { "rank": int, "feature": { "id": "uuid", "name": "string", "description": "string" }, "score": float, "variance": float, "confidence_interval": [float, float] } ]` |
