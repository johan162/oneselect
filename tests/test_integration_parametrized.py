...
class TestParameterizedWorkflow:
    ...
    def test_parameterized_workflow(
        ...
        print(f"\n{'='*70}")
        print(
            f"TEST: {num_features} features, {target_confidence:.0%} target confidence"
        )
        print(f"{'='*70}")
        ...
        while complexity_comparisons < max_iterations:
            ...
            if r.status_code == 204:
                ...
                print(
                    f"  Complexity: Target {target_confidence:.0%} reached after {complexity_comparisons} comparisons"
                )
                break
        ...
        while value_comparisons < max_iterations:
            ...
            if r.status_code == 204:
                # Target certainty reached
                print(
                    f"  Value: Target {target_confidence:.0%} reached after {value_comparisons} comparisons"
                )
                break
        ...
        print(
            f"✓ Total comparisons: {total_comparisons} ({complexity_comparisons} complexity + {value_comparisons} value)"
        )
        ...
        complexity_ranking = [
            result["feature"]["name"] for result in complexity_results
        ]
        expected_complexity = sorted(
            feature_names, key=lambda x: complexity_order[x], reverse=True
        )

        complexity_inversions = calculate_inversions(
            complexity_ranking, expected_complexity
        )
        ...
        expected_value = sorted(
            feature_names, key=lambda x: value_order[x], reverse=True
        )
        ...
        print(f"\n{'─'*70}")
        print(
            f"📊 RESULTS SUMMARY: {num_features} features, {target_confidence:.0%} target"
        )
        print(f"{'─'*70}")
        ...
        print(
            f"    Value:       {value_comparisons:4d} / {total_pairs} pairs ({value_comparisons/total_pairs:.1%})"
        )
        ...
        print(
            f"    Value:      {value_inversions} inversions, goodness={value_goodness:.4f}"
        )
        ...
        if complexity_reached_target:
            print(
                f"✓ Complexity reached target confidence: {complexity_achieved:.1%} >= {target_confidence:.0%}"
            )
        else:
            print(
                f"ℹ Complexity stopped at {complexity_achieved:.1%} (target: {target_confidence:.0%}) - ranking complete via transitivity"
            )

        if value_reached_target:
            print(
                f"✓ Value reached target confidence: {value_achieved:.1%} >= {target_confidence:.0%}"
            )
        else:
            print(
                f"ℹ Value stopped at {value_achieved:.1%} (target: {target_confidence:.0%}) - ranking complete via transitivity"
            )
        ...
        print(
            f"   Goodness scores: complexity={complexity_goodness:.4f}, value={value_goodness:.4f}"
        )
        ...
        _test_results.append(
            {
                ...
                "complexity_transitive_coverage": complexity_progress[
                    "transitive_coverage"
                ],
...
def print_summary_table():
    ...
    sorted_results = sorted(
        _test_results, key=lambda x: (x["num_features"], -x["target_confidence"])
    )
    ...
    # Header
    print(
        f"{'Features':<10} {'Target':<8} {'Total':<8} {'Cmplx':<8} {'Value':<8} {'Cmplx':<10} {'Value':<10} {'Cmplx':<10} {'Value':<10} {'Avg':<10}"
    )
    print(
        f"{'':10} {'Conf.':<8} {'Pairs':<8} {'Comp.':<8} {'Comp.':<8} {'Goodness':<10} {'Goodness':<10} {'Eff.Conf':<10} {'Eff.Conf':<10} {'Goodness':<10}"
    )
    print("-" * 120)

    for r in sorted_results:
        print(
            f"{r['num_features']:<10} {r['target_confidence']:.0%}{'':4} {r['total_pairs']:<8} "
    ...
    print("=" * 120)
    print(
        f"{'Features':<10} {'Target':<8} {'Cmplx Direct':<14} {'Cmplx Trans.':<14} {'Value Direct':<14} {'Value Trans.':<14} {'Cmplx Inv.':<12} {'Value Inv.':<12}"
    )
    print("-" * 120)

    for r in sorted_results:
        print(
            f"{r['num_features']:<10} {r['target_confidence']:.0%}{'':4} "
    ...
    for i, r1 in enumerate(sorted_results):
        for r2 in sorted_results[i + 1 :]:
            if (
                r1["num_features"] == r2["num_features"]
                and r1["target_confidence"] > r2["target_confidence"]
                and r1["avg_goodness"] < r2["avg_goodness"]
            ):
    ...
    if anomalies:
        ...
        for r1, r2 in anomalies:
            print(
                f"   • {r1['num_features']} features: {r1['target_confidence']:.0%} target → {r1['avg_goodness']:.4f} goodness"
            )
            print(
                f"                      {r2['target_confidence']:.0%} target → {r2['avg_goodness']:.4f} goodness (BETTER by {r2['avg_goodness'] - r1['avg_goodness']:.4f})"
            )
            print(
    ...
    full_coverage_cases = [
        r for r in sorted_results if r["value_comparisons"] >= r["total_pairs"] * 0.95
    ]
    if full_coverage_cases:
        print("\nℹ️  VALUE DIMENSION REQUIRED NEAR-FULL COVERAGE in these cases:")
        for r in full_coverage_cases:
            print(
                f"   • {r['num_features']} features, {r['target_confidence']:.0%} target: {r['value_comparisons']}/{r['total_pairs']} pairs ({r['value_comparisons']/r['total_pairs']:.1%})"
            )
        print(
            "   Reason: Value comparisons run AFTER complexity, starting with fresh transitive knowledge."
        )
    ...
    for n in [10, 20, 30]:
        ...
        if n_results:
            ...
            print(
                f"   • {n} features ({total_pairs} pairs): avg={avg_total:.0f}, min={min_total}, max={max_total} comparisons"
            )
