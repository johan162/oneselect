...
class TestIntegrationWorkflow:
    ...
    def test_complete_workflow(
        ...
        assert r.status_code == 200
        estimates = r.json()
        print(f"✓ Comparison estimates for 90% confidence: {estimates['estimates']['90%']}")
        ...
        while complexity_comparisons < max_iterations:
            ...
            if r.status_code == 204:
                ...
                print(f"✓ Complexity: Ranking complete! (all pairs compared, 204 returned)")
                break
            ...
            assert r.status_code == 201, f"Failed comparison {feat_a_name} vs {feat_b_name}: {r.text}"
            ...
            print(f"  Complexity: {complexity_comparisons} comparisons, coverage={coverage:.0%}, effective={effective:.0%}")
        ...
        while value_comparisons < max_iterations:
            ...
            coverage = progress["coverage_confidence"]
            effective = progress["effective_confidence"]
            print(f"  Value: {value_comparisons} comparisons, coverage={coverage:.0%}, effective={effective:.0%}")
        ...
        print("-" * 70)
        print(f"{'Feature':<10} {'Complexity μ':>12} {'Complexity σ':>12} {'Value μ':>12} {'Value σ':>12}")
        print("-" * 70)
        ...
        for f in features:
            ...
            print(f"{name:<10} {complexity_mu:>12.4f} {complexity_sigma:>12.4f} {value_mu:>12.4f} {value_sigma:>12.4f}")
        ...
        complexity_goodness = calculate_goodness_score(complexity_ranking, expected_complexity_order)
        print(f"  Goodness score: {complexity_goodness:.2%}")
        assert complexity_goodness >= 0.8, \
        ...
        assert "FeatE" in quick_wins, f"FeatE should be a Quick Win, but quadrants are: {quadrants}"
        print("  ✓ FeatE correctly identified as Quick Win!")
        
        # Verify FeatA is Avoid (highest complexity, lowest value)
        assert "FeatA" in avoid, f"FeatA should be Avoid, but quadrants are: {quadrants}"
        ...
        print("-" * 60)
        print(f"  Complexity:")
        print(f"    Direct Coverage:   {complexity_progress['direct_coverage']:.0%} ({complexity_progress['unique_pairs_compared']}/{complexity_progress['total_possible_pairs']} pairs)")
        print(f"    Transitive Coverage: {complexity_progress['transitive_coverage']:.0%} ({complexity_progress['transitive_known_pairs']}/{complexity_progress['total_possible_pairs']} pairs)")
        print(f"    Bayesian:   {complexity_progress['bayesian_confidence']:.2%}")
        print(f"    Consistency: {complexity_progress['consistency_score']:.0%} ({complexity_progress['cycle_count']} cycles)")
        print(f"    Effective:  {complexity_progress['effective_confidence']:.0%}")
        print(f"  Value:")
        print(f"    Direct Coverage:   {value_progress['direct_coverage']:.0%} ({value_progress['unique_pairs_compared']}/{value_progress['total_possible_pairs']} pairs)")
        print(f"    Transitive Coverage: {value_progress['transitive_coverage']:.0%} ({value_progress['transitive_known_pairs']}/{value_progress['total_possible_pairs']} pairs)")
        print(f"    Bayesian:   {value_progress['bayesian_confidence']:.2%}")
        print(f"    Consistency: {value_progress['consistency_score']:.0%} ({value_progress['cycle_count']} cycles)")
        ...
        if complexity_comparisons < max_comparisons:
            print(f"  ✓ Transitive optimization saved {max_comparisons - complexity_comparisons} complexity comparisons!")
        if value_comparisons < max_comparisons:
            print(f"  ✓ Transitive optimization saved {max_comparisons - value_comparisons} value comparisons!")

        print(f"  ✓ Comparisons per dimension: complexity={complexity_comparisons}, value={value_comparisons}")
        ...
        print(f"  • Total comparisons: {total_comparisons} (max would be {max_comparisons * 2})")
...
def test_theoretical_score_verification(
    ...
    print("-" * 40)
    for name in feature_names:
        print(f"  {name}: score={scores[name]:.4f}, expected_rank={true_strength[name]}")
    
    # Verify ordering matches true strength
    sorted_by_score = sorted(scores.keys(), key=lambda x: scores[x], reverse=True)
    sorted_by_truth = sorted(true_strength.keys(), key=lambda x: true_strength[x], reverse=True)
    
    print(f"\n  Score order:    {sorted_by_score}")
    print(f"  Expected order: {sorted_by_truth}")
    
    assert sorted_by_score == sorted_by_truth, \
